...
ProcessHandle_Win::ProcessHandle_Win(QString executablePath) :
        ProcessHandle(std::move(executablePath)) {
}

ProcessHandle_Win::~ProcessHandle_Win() {
    if (processWaiter != nullptr) {
        processWaiter->requestInterruption();
        processWaiter->deleteLater();
    }
...
HANDLE ProcessHandle_Win::findProcess() {
    ...
    if (Process32First(snapshot, &entry) == TRUE) {
        ...
        while (Process32Next(snapshot, &entry) == TRUE) {
            if (exeName.compare(entry.szExeFile, Qt::CaseInsensitive) == 0) {
...
#ifdef UNICODE
        errorMsg = QString::fromStdWString(errorMsgBuffer);
#else
        errorMsg = errorMsgBuffer;
#endif /* UNICODE */
        LocalFree(errorMsgBuffer);
    }
    emit processHandle->finished(ExitReason(waitResult == WAIT_FAILED, errorMsg));
    this->deleteLater();
